...
manager_llm = HuggingFaceEndpoint(
...
# Check if MDXSearchTool can be configured to use Hugging Face
try:
    semantic_search_resume = MDXSearchTool(mdx='Resume.md', embedding_model=HuggingFaceEndpoint(
        repo_id="HuggingFaceH4/zephyr-7b-beta",
        huggingfacehub_api_token=os.getenv("hf_api"),
        task="embedding"  # Assuming an embedding task if available
    ))
except Exception as e:
    print(f"Error initializing MDXSearchTool: {e}")
...
researcher = Agent(
    ...
    verbose=True,
    backstory=(
        "As a Job Researcher, your prowess in navigating and extracting critical information "
        "from job postings is unmatched. Your skills help pinpoint the necessary qualifications "
        "and skills sought by employers, forming the foundation for effective application tailoring."
    ),
...
profiler = Agent(
    ...
    backstory=(
        ...
        "to craft comprehensive personal and professional profiles, laying the groundwork for personalized resume enhancements."
    ),
    manager_llm=manager_llm
)
...
interview_preparer = Agent(
    ...
    backstory=(
        ...
        "and talking points, you prepare candidates for success, ensuring they can confidently address all aspects of the job they are applying for."
    ),
    manager_llm=manager_llm
)
...
resume_strategy_task = Task(
    description=(
        ...
        "Update every section, including the initial summary, work experience, skills, and education, to better reflect the candidate's abilities and how it matches the job posting."
    ),
    expected_output=(
...
job_application_inputs = {
    ...
    'github_url': 'https://github.com/Gangadhar24377',
    'personal_writeup': """ I am Gangadhar Kambhamettu, Highly motivated and accomplished STEM student with a specialization in Artificial Intelligence and Machine Learning, showcasing a strong proficiency in IoT, React, and Django. 

Equipped with hands-on experience in cutting-edge technologies and a passion for solving complex problems through innovative solutions. 

Adept at working in multidisciplinary teams, contributing to projects, and continuously expanding knowledge in the fast-paced tech landscape.
"""
}
